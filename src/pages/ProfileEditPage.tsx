  ...
  useEffect(() => {
    const loadProfile = async () => {
      ...
      const { data, error } = await supabase
        .from('profiles')
        .select(
          'id, full_name, username, bio, created_at, minecraft_username, cover_image'
        )
        .eq('id', user.id)
        .maybeSingle();
            ...
            <div className="pt-4">
              <div className="rounded-2xl border border-purple-500/20 bg-purple-500/[0.04] p-5">
                <div className="mb-4 flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-xs text-slate-500">
                      Upload a cover image for your profile (recommended size: 1920x400px)
                    </p>
                    <p className="mt-1 text-xs text-purple-400">
                      Free image hosting options: Imgur (imgur.com), PostImg (postimg.cc), Cloudinary (cloudinary.com)
                    </p>
                  </div>
                </div>
                ...
                {(coverImage || coverPreviewUrl) && (
                  <div className="mb-4">
                    <p className="mb-2 text-xs text-slate-500">Current Cover Image:</p>
                    <div className="relative h-32 w-full overflow-hidden rounded-xl border border-purple-500/30 bg-black/40">
                      <img
                ...
                <p className="mt-2 text-xs text-slate-500">
                  Upload to a free image hosting service and paste the direct image URL here.
                </p>

                {/* Free Image Hosting Suggestions */}
                <div className="mt-4 rounded-xl border border-blue-500/20 bg-blue-500/[0.04] p-4">
                  <p className="text-xs font-medium text-blue-400 mb-2">Quick Free Hosting Options:</p>
                  <div className="space-y-2 text-xs text-slate-300">
                    <div>
                      <strong>Imgur:</strong> Upload to imgur.com → Get "Direct link" → Paste here
                    </div>
                    <div>
                      <strong>PostImg:</strong> Upload to postimg.cc → Copy "Direct image link" → Paste here
                    </div>
                    <div>
                      <strong>Cloudinary:</strong> Upload to cloudinary.com → Copy "Secure URL" → Paste here
                    </div>
                  </div>
                </div>
                ...
                <button
                  type="button"
                  onClick={async () => {
                    if (!profile || !coverImage.trim()) return;
                    ...
                    try {
                      ...
                      if (error || !data) {
                        setMessage(`Could not save cover image: ${error?.message || 'Unknown error'}`);
                      ...
                      setMessage('Cover image saved successfully!');
                    } catch (err) {
                      setMessage(`Could not save cover image: ${(err as Error).message}`);
                    } finally {
                      setSavingCover(false);
                    }
                  }}
                  disabled={savingCover || !coverImage.trim()}
                  className="w-full flex items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-purple-600 to-indigo-600 py-3 font-semibold transition-all hover:from-purple-500 hover:to-indigo-500 disabled:cursor-not-allowed disabled:opacity-50 mt-4"
                >
                  {savingCover ? (
                    <>
